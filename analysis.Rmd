...
df_clean %>% 
  agg(id = stdid, es = ES, var = EV, method = "BHHR", data = .)
...
grouped_cv <- trainControl(method = "cv", index = groupKFold(df_v$stdid, k = 10))
tuning_grid <- expand.grid(whichweights = c("random", "fixed", "unif"), mtry = 2:6, min.node.size = 2:6)
X <- df_v[, c("stdid", "EV", retain_mods)] %>% rename(vi = EV)
mf_cv <- train(y = df_v$ES,
               x = X,
               study = "stdid",
               method = ModelInfo_mf(), 
