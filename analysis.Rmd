...
df_append %>% 
  mutate(Selection = if_else(RCT == 1, "L", "H")) %>% 
  ...
  rename(Study = stdid, `Selection bias` = Selection, `Performance bias` = Performance, `Detection bias` = Detection, `Attrition bias` = Attrition, `Reportiong bias` = Reporting) %>%
  left_join(
    cor_es %>%
      group_by(stdid) %>%
      summarise(EV = mean(EV, na.rm = T)) %>%
      transmute(Study = stdid, Overall = "L", Weight = 1/EV),
    by = "Study"
  )
