...
#### Comparing Across Outcomes (Decoding/ReadingComp/LanguageComp/Writing)

```{r}
df_clean %>%
  mutate(
    outcome = case_when(
      str_detect(type, "^D") ~ "Decoding",
      str_detect(type, "^C") ~ "Reading Comp",
      str_detect(type, "^L") ~ "Language Comp",
      str_detect(type, "^W") ~ "Writing",
      TRUE ~ NA_character_
    ) %>% 
    factor(levels = c("Decoding", "Reading Comp", "Language Comp", "Writing")),
  ) %>% 
