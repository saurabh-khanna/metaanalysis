...
# generate this table later after combining in direct ESs
# df_raw %>%
#   mutate(
...
Study and measure overview [223 total measures across 120 studies]:
...
Effect size and its variance for all 223 study measures:
...
#   relocate(AUTYR, stdid) %>%
#   arrange(AUTYR, type) %>% 
#   relocate(`Study ID` = stdid, `Author (Year)` = AUTYR, `Outcome type` = type, `Effect Size` = ES, `Effect Size Variance` = EV) %>%
#   clean_names(case = "title") %>% 
#   mutate_if(is.numeric, ~ round(., 4)) %>% 
#   writexl::write_xlsx(here::here("data_ed_tech_meta.xlsx"))
```
...
```{r, eval=F}
df_l %>% filter(str_detect(AUTYR, "Korat")) %>% arrange(AUTYR)
```
