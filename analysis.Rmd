...
df_raw <-
  ...
  remove_empty() %>%
  select(-`Author (Year)`) %>% 
  select(study_id, type, sort(tidyselect::peek_vars()))
```
...
df_hlm1 <-
  ...
  rename(study_id = `STUDYID`) %>%
  drop_na(study_id) %>%
  select(study_id, type, msr, sort(tidyselect::peek_vars())) %>% 
  remove_empty() %>% 
...
# df_hlm %>% distinct(study_id)
```


```{r}
df_raw <- bind_rows(df_raw, df_hlm) %>% distinct()
...
# anti_join(df_post, df_prepost, by = c("study_id", "AUTYR", "type", "num"))
```
...
df_clean <-
  df_clean %>%
  inner_join(moderators, by = "stdid") %>%
...
- ICCs decided based on cluster size and grade level from Table 3 [here](https://journals.sagepub.com/doi/epub/10.3102/0162373707299706).
- Table 4 in [this paper](https://link.springer.com/article/10.1007/s11145-013-9455-2/tables/4) reports very similar ICCs.
- Also in above paper: "Though there is no agreed upon delineation of what constitutes a large ICC, several authors have noted that school level variation often ranges from .05 to .20 (Muthén & Satorra, 1989; Spybrook, Raudenbush, Congdon, & Martinez, 2009). Hox (2010) has put forth that ICCs of .05, .10, and .15 can be viewed as small, moderate, or large estimates."
...
# hlm modifications for standard errors

df_c <-
  df_c %>% 
  mutate(
    EV = case_when(
      stdid == "95" ~ EV * (1 + (20 - 1) * 0.242),
      stdid == "96" ~ EV * (1 + (20 - 1) * 0.242),
      stdid == "97" ~ EV * (1 + (20 - 1) * 0.263),
      stdid == "98" ~ EV * (1 + (20 - 1) * 0.263),
      stdid == "99" ~ EV * (1 + (6 - 1) * 0.242),
      stdid == "100" ~ EV * (1 + (9 - 1) * 0.263),
      stdid == "101" ~ EV * (1 + (5 - 1) * 0.242),
      stdid == "102" ~ EV * (1 + (5 - 1) * 0.263),
      stdid == "74" ~ EV * (1 + (15 - 1) * 0.239),
      TRUE ~ EV
    )
  )
...
df_d <-
  ...
  mutate(
    EV = case_when(
      stdid == "61" ~ EV * (1 + (8 - 1) * 0.239),
      stdid == "74" ~ EV * (1 + (14 - 1) * 0.239),
      TRUE ~ EV
    )
  )
...
df_clean <- bind_rows(df_c, df_d, df_l, df_w) %>% arrange(type, stdid, AUTYR)
```



Study and measure overview [224 total measures across 120 studies]:
...
Effect size and its variance for all 224 study measures:
...
df_d %>% 
  ...
  skim() %>%
  as_tibble() %>% 
  select(moderator = skim_variable, top_counts = factor.top_counts, n_unique = factor.n_unique, n_missing) %>% 
  arrange(moderator)
```
...
### Variation with Imputed ICC

```{r}
# Define the function
process_and_plot_model <- function(df) {
  # Initialize a dataframe to store results
  results <- data.frame(ICC = numeric(), Beta = numeric(), CI_Lower = numeric(), CI_Upper = numeric())

  # Loop over ICC values
  for (icc in seq(0, 1, by = 0.01)) {
    ...
    temp_model <- df %>%
      ...
      mutate(
        EV = case_when(
          stdid == "61" ~ EV * (1 + (8 - 1) * icc),
          stdid == "74" ~ EV * (1 + (14 - 1) * icc),
          TRUE ~ EV
        )
      ) %>%
      rma(
    ...
    # Extract beta coefficient and standard error
    beta <- coef(temp_model)[1]
    se <- sqrt(diag(vcov(temp_model)))[1]
    ci_lower <- beta - 1.96 * se
    ci_upper <- beta + 1.96 * se
    
    # Store results
    results <- rbind(results, data.frame(ICC = icc, Beta = beta, CI_Lower = ci_lower, CI_Upper = ci_upper))
  }
  
  # Plotting
  ggplot(results) +
    geom_rect(data = df[1,], aes(xmin = 0.2, xmax = 0.27, ymin = -Inf, ymax = Inf), fill = "green", alpha = 0.4) +
    geom_ribbon(aes(x = ICC, y = Beta, ymin = CI_Lower, ymax = CI_Upper), alpha = 0.2) +
    geom_line(aes(x = ICC, y = Beta), color = "blue") +
    #geom_vline(xintercept = c(0.2, 0.27), linetype="dashed", color = "blue") +
    coord_cartesian(ylim = c(0, 0.45)) +
    scale_x_continuous(breaks = scales::breaks_width(0.1), minor_breaks = NULL) +
    scale_y_continuous(breaks = scales::breaks_width(0.05), minor_breaks = NULL) +
    labs(
      title = "Variation of sythesized effect size with imputed ICC values",
      subtitle = "Green band shows ICC values for reading from prior literature (Hedges & Hedberg, 2007).\nGrey band represents 95% confidence interval around the synthesized effect size.",
      x = "Intra-class correlation coefficient (ICC)",
      y = "Synthesized effect size"
    ) +
    theme_minimal()
}
...
process_and_plot_model <- function(df) {
  ...
  for (icc in seq(0, 1, by = 0.01)) {
    ...
    temp_model <- df %>%
      ...
      mutate(
        EV = case_when(
          stdid == "95" ~ EV * (1 + (20 - 1) * icc),
          stdid == "96" ~ EV * (1 + (20 - 1) * icc),
          stdid == "97" ~ EV * (1 + (20 - 1) * icc),
          stdid == "98" ~ EV * (1 + (20 - 1) * icc),
          stdid == "99" ~ EV * (1 + (6 - 1) * icc),
          stdid == "100" ~ EV * (1 + (9 - 1) * icc),
          stdid == "101" ~ EV * (1 + (5 - 1) * icc),
          stdid == "102" ~ EV * (1 + (5 - 1) * icc),
          stdid == "74" ~ EV * (1 + (15 - 1) * icc),
          TRUE ~ EV
        )
      ) %>% 
      rma(
...
# df_l %>% mutate(type = factor(type, levels = c("LRP", "LSN", "LRD"))) %>% count(type)
```
...
## Verifying effect sizes using RVE (Robust Variance Estimation)
...
```{r}
robu(ES ~ 1, data = df_d, studynum = stdid, var.eff.size = EV, modelweights = "HIER", small = TRUE)
```
...
```{r}
robu(ES ~ 1, data = df_c, studynum = stdid, var.eff.size = EV, modelweights = "HIER", small = TRUE)
```
...
```{r}
robu(ES ~ 1, data = df_l, studynum = stdid, var.eff.size = EV, modelweights = "HIER", small = TRUE)
```
...
```{r}
robu(ES ~ 1, data = df_w, studynum = stdid, var.eff.size = EV, modelweights = "CORR", small = TRUE)
```
...
```{r}
df_d %>%
  mutate(type = as_factor(type) %>% fct_rev()) %>% 
  rma(yi = ES, vi = EV, data = ., method = "REML", mods = ~ type)
```
...
```{r, results='asis'}
df_temp <- df_d %>% mutate(type = as_factor(type) %>% fct_rev())

lm2 <- rma(yi = ES, vi = EV, data = df_temp, method = "REML", mods = ~ type)

lm3 <- rma(yi = ES, vi = EV, data = df_temp, method = "REML", mods = ~ type + avghrs_categories)

lm4 <- rma(yi = ES, vi = EV, data = df_temp, method = "REML", mods = ~ type + avghrs_categories + pedagogy)

lm5 <- rma(yi = ES, vi = EV, data = df_temp, method = "REML", mods = ~ type + avghrs_categories + pedagogy + grade)

lm6 <- rma(yi = ES, vi = EV, data = df_temp, method = "REML", mods = ~ type + avghrs_categories + pedagogy + grade + low_income_included)

knitreg(list(lm2, lm3, lm4, lm5, lm6), include.ci = FALSE, dcolumn = TRUE, booktabs = TRUE, digits = 3, caption = "")
```
...
```{r, results='asis'}
df_temp <- df_c %>% mutate(type = as_factor(type) %>% fct_rev())
...
```{r, results='asis'}
df_temp <- df_l %>% mutate(type = as_factor(type) %>% fct_rev())
...
```{r}
rma(yi = ES, vi = EV, data = df_d %>% filter(typetech != "Other"), method = "REML", mods = ~ typetech)
```
...
```{r}
rma(yi = ES, vi = EV, data = df_d %>% filter(typetech != "ER"), method = "REML", mods = ~ typetech)
```
...
```{r}
rma(yi = ES, vi = EV, data = df_d %>% filter(typetech != "CAI"), method = "REML", mods = ~ typetech)
```
...
```{r}
rma(yi = ES, vi = EV, data = df_l %>% filter(typetech != "Other"), method = "REML", mods = ~ typetech)
```
...
```{r}
rma(yi = ES, vi = EV, data = df_l %>% filter(typetech != "ER"), method = "REML", mods = ~ typetech)
```
...
```{r}
rma(yi = ES, vi = EV, data = df_l %>% filter(typetech != "CAI"), method = "REML", mods = ~ typetech)
```
...
```{r}
rma(yi = ES, vi = EV, data = df_c %>% filter(typetech != "Other"), method = "REML", mods = ~ typetech)
```
...
```{r}
rma(yi = ES, vi = EV, data = df_c %>% filter(typetech != "ER"), method = "REML", mods = ~ typetech)
```
...
```{r}
rma(yi = ES, vi = EV, data = df_c %>% filter(typetech != "CAI"), method = "REML", mods = ~ typetech)
```
...
```{r, results='asis'}
df_d <- df_d %>% mutate(type = as_factor(type) %>% fct_rev())

lm1 <- rma(yi = ES, vi = EV, data = df_d, method = "REML", mods = ~ type + pedagogy)

lm2 <- rma(yi = ES, vi = EV, data = df_d, method = "REML", mods = ~ type + avghrs_categories)

lm3 <- rma(yi = ES, vi = EV, data = df_d, method = "REML", mods = ~ type + avghrs_categories + pedagogy)

knitreg(list(lm1, lm2, lm3), include.ci = FALSE, dcolumn = TRUE, booktabs = TRUE, digits = 3, caption = "")
```

**Language-focused:**

```{r, results='asis'}
df_l <- df_l %>% mutate(type = as_factor(type) %>% fct_rev())

lm1 <- rma(yi = ES, vi = EV, data = df_l, method = "REML", mods = ~ type + pedagogy)

lm2 <- rma(yi = ES, vi = EV, data = df_l, method = "REML", mods = ~ type + avghrs_categories)

lm3 <- rma(yi = ES, vi = EV, data = df_l, method = "REML", mods = ~ type + avghrs_categories + pedagogy)
...
```{r, results='asis'}
df_c <- df_c %>% mutate(type = as_factor(type) %>% fct_rev())

lm1 <- rma(yi = ES, vi = EV, data = df_c, method = "REML", mods = ~ type + pedagogy)

lm2 <- rma(yi = ES, vi = EV, data = df_c, method = "REML", mods = ~ type + avghrs_categories)

lm3 <- rma(yi = ES, vi = EV, data = df_c, method = "REML", mods = ~ type + avghrs_categories + pedagogy)
...
```{r}
df_clean %>%
  distinct(stdid, pedagogy, gamification) %>% 
  mutate(n = 1) %>% 
  group_by(pedagogy) %>%
  summarise(
    gamification = round(mean(gamification, na.rm = T) * 100, 2) %>% as.character() %>% str_c("%"),
    count = sum(n, na.rm = T)
  )
```
